...
# How My AFK Engineering Workflow Actually Works
...
For small, bounded changes and bug fixes, I still interact directly with the
agent rather than sending everything through the full AFK pipeline.

Sometimes the overhead of the complete workflow is simply not justified. If
the change is small enough, it is faster and clearer for me to stay in the
loop, give the agent the context it needs, review what it does, and make the
decision directly.
